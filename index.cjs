...
const SYDNEY_SUBURBS = [
  ...
  "Yagoona",
  "Zetland",
  // 👉 You can paste the full list later
];
...
try {
  ...
  console.error("❌ suburbFuse failed:", e?.message || e);
}
...
function bestSydneySuburb(raw) {
  ...
  if (best.score != null && best.score <= 0.35) {
    return best.item;
  }
...
app.post("/afterhours", async (req, res) => {
  ...
  if (openai && speech) {
    try {
      ...
      // Fix suburb using Sydney fuzzy matching
const snapped = bestSydneySuburb(extracted.location);
if (snapped) {
  console.log("📍 Suburb corrected:", extracted.location, "→", snapped);
  extracted.location = snapped;
}
    } catch (e) {
